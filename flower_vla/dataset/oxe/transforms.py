...
from flower_vla.dataset.utils.droid_utils import rand_swap_exterior_images

from flower_vla.dataset.utils.data_utils import (
...
def format_instruction(
    ...
    Args:
        ...
        prompt_style (str): Style of the prompt. Default is "minimal".
    ...
    def format_single_instruction(instr):
        ...
        # If prompt_style is 'default', return the instruction as-is
...
def generate_policy_prompt(
    ...
    Args:
        ...
        prompt_style: Prompt generation strategy ("default", "combined", "structured", "visual", "minimal")
    ...
    prompts = {
        # Default option that returns just the instruction
        "default": instruction,
    ...
    # Only apply whitespace cleanup if not using default style
    if prompt_style != "default":
...
def kit_irl_dataset_delta_joint_transform(trajectory: Dict[str, Any]) -> Dict[str, Any]:
...
def droid_dataset_transform(trajectory: Dict[str, Any]) -> Dict[str, Any]:
    """
    DROID dataset transformation for processing trajectory data.
    """
    trajectory["action"] = tf.concat(
        [
    ...
    trajectory['language_instruction'] = format_instruction( trajectory['language_instruction'], "Franka", "1", "Delta End-Effector", prompt_style=PROMPT_STYLE, )
    trajectory["language_instruction_2"] = format_instruction( trajectory['language_instruction_2'], "Franka Panda", "1", "Delta End-Effector", prompt_style=PROMPT_STYLE,)
    trajectory["language_instruction_3"] = format_instruction(trajectory['language_instruction_3'], "Franka Panda", "1","Delta End-Effector", prompt_style=PROMPT_STYLE,)
...
def delta_droid_dataset_transform(trajectory: Dict[str, Any]) -> Dict[str, Any]:
    """
    DROID dataset transformation for processing trajectory data.
    Computes velocities from delta positions at 15Hz control frequency.
    """
    # not used: Convert delta positions to velocities by dividing by dt
    joint_velocity = trajectory["action_dict"]["joint_velocity"]

    # Combine joint velocities with gripper action
    trajectory["action"] = tf.concat(
        [
            joint_velocity,  # Joint velocities for 7 joints
            binarize_gripper_actions(trajectory["action_dict"]["gripper_position"][:, -1], 0.95, 0.05)[:, None],
        ],
        axis=-1,
    )
    
    # Format language instructions specifying Delta End-Effector control
    ...
    # Update robot information to specify delta joint control
    trajectory["robot_information"] = add_robot_information("Franka", "delta joint", 1)
    # Update action space index for delta joint control
    trajectory['action_space_index'] = get_action_space_index('JOINT_POS', 1, 'velocity')
...
def bridge_transform(trajectory: Dict[str, Any]) -> Dict[str, Any]:
    ...
    trajectory["robot_information"] = add_robot_information("WidowX", "delta end-effector", 1)
...
def aloha_sim_insertion_dataset_transform(trajectory: Dict[str, Any]) -> Dict[str, Any]:
    print(trajectory.keys())
    trajectory["observation"]["proprio"] = trajectory["observation"]["state"]
    # trajectory["observation"]["images_top"] = tf.cast(trajectory["observation"]["images_top"], tf.float32) / 255.0
...
def aloha_sim_transfer_dataset_transform(trajectory: Dict[str, Any]) -> Dict[str, Any]:
    print(trajectory.keys())
    trajectory["observation"]["proprio"] = trajectory["observation"]["state"]
    # trajectory["observation"]["images_top"] = tf.cast(trajectory["observation"]["images_top"], tf.float32) / 255.0
    trajectory["robot_information"] = add_robot_information("ViperX", "absolute joint", 2)
...
def toto_dataset_transform_joint(trajectory: Dict[str, Any]) -> Dict[str, Any]:
    ...
    traj_truncated["robot_information"] = add_robot_information("Franka", "absolute joint", 1)
...
def calvin_dataset_transform(trajectory: Dict[str, Any]) -> Dict[str, Any]:
    ...
    proprio_index = [0, 1, 2, 3, 4, 5, 6, 7, 14]
    trajectory["observation"]["proprio"] = tf.gather(trajectory["observation"]["state"], indices=proprio_index, axis=-1)
    trajectory["observation"]["EEF_state"] = trajectory["observation"]["state"][:, :6]
    trajectory["observation"]["gripper_state"] = trajectory["observation"]["state"][:, -2:]  # 2D gripper state
    trajectory["robot_information"] = add_robot_information("Franka", "delta end-effector", 1)
    trajectory['action_space_index'] = get_action_space_index('EEF_POS', 1, 'velocity')
...
OXE_STANDARDIZATION_TRANSFORMS = {
    ...
    "bridge_dataset_without_single": bridge_dataset_transform,
    "kit_irl_real_kitchen_lang_delta": kit_irl_dataset_delta_joint_transform, # old
    "kit_irl_real_kitchen_vis": kit_irl_dataset_abs_joint_transform, 
    ...
    "delta_droid": delta_droid_dataset_transform,
    ...
    "toto": toto_dataset_transform_eef,
