...
from flower_vla.dataset.utils.data_utils import tree_merge
...
def bound_uniform(traj: dict, min_bound: Optional[int] = None, max_bound: Optional[int] = None) -> dict:
    """
    Relabels with a true uniform distribution over future states.
    Optionally caps goal distance.
    """
    ...
    if min_bound is not None:
        low = tf.cast(
            tf.minimum(tf.range(traj_len) + min_bound, traj_len), tf.float32
        )
    else:
        low = tf.cast(tf.range(traj_len), tf.float32)
    if max_bound is not None:
        high = tf.cast(
            tf.minimum(tf.range(traj_len) + max_bound, traj_len), tf.float32
        )
    else:
        high = tf.cast(traj_len, tf.float32)
    goal_idxs = tf.cast(rand * (high - low) + low, tf.int32)
...
def bound_uniform_and_future(traj: dict, min_bound: Optional[int] = None, max_bound: Optional[int] = None, frame_diff: int = 1) -> dict:
    """
    Relabels with a true uniform distribution over future states.
    Optionally caps goal distance.
    """
