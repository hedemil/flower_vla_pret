...
from flower_vla.dataset.utils.dataset_index import DATASET_NAME_TO_INDEX
...
class RLDSProcessing:
    ...
    def process_robot_information(old_obs: dict, traj_len: int) -> tf.Tensor:
        ...
        try:
            # Check if robot_information exists in the observation
            if 'robot_information' in old_obs:
                ...
                # If not present, return placeholder - it will be filled by transform later
                return tf.repeat("", repeats=[traj_len])
                
        except Exception as e:
            # Log warning instead of error
            logging.warning(f"Issue processing robot information: {e}")
            return tf.repeat("", repeats=[traj_len])
    ...
    @staticmethod
    def process_language_embedding(traj: dict, traj_len: int) -> tf.Tensor:
        """
        Process language embeddings from trajectory data, ensuring proper shape and type.
        
        Args:
            traj (dict): The trajectory dictionary containing language embeddings
            traj_len (int): Length of the trajectory for proper tensor tiling
            
        Returns:
            tf.Tensor: Processed language embeddings with shape [traj_len, embedding_dim]
            
        Notes:
            - Language embeddings are expected to be a single vector per trajectory
            - We tile this vector across the trajectory length to match other features
            - Input shape is (None, 512) where None is typically 1
            - Output shape will be (traj_len, 512)
        """
        try:
            # Get the language embedding and ensure it's the right type
            language_embedding = tf.cast(traj['language_embedding'], tf.float32)
            
            # Remove any extra dimensions (in case of shape [1, None, 512])
            language_embedding = tf.squeeze(language_embedding)
            
            # If embedding is just a single vector, expand it to match trajectory length
            if len(language_embedding.shape) == 1:
                # Add batch dimension and tile
                language_embedding = tf.tile(
                    tf.expand_dims(language_embedding, 0),
                    [traj_len, 1]
                )
            elif len(language_embedding.shape) == 2 and tf.shape(language_embedding)[0] == 1:
                # If we have shape [1, 512], tile it
                language_embedding = tf.tile(language_embedding, [traj_len, 1])
            elif len(language_embedding.shape) == 2 and tf.shape(language_embedding)[0] != traj_len:
                raise ValueError(
                    f"Language embedding length {tf.shape(language_embedding)[0]} "
                    f"doesn't match trajectory length {traj_len}"
                )
                
            # Verify final shape is correct
            tf.debugging.assert_shapes([
                (language_embedding, ('N', 512))  # N should match traj_len
            ])
            
            return language_embedding
            
        except Exception as e:
            logging.warning(f"Error processing language embedding: {e}")
            # Return zero embeddings if processing fails
            return tf.zeros([traj_len, 512], dtype=tf.float32)
    ...
    def process_language(
        ...
        if "language_embedding" in traj:  # or whatever the embedding key is
            task["language_embedding"] = traj["language_embedding"]
        else:
            print("No language embeddings found")
        return task, local_lang_key
